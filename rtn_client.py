...
def api_find_devices(
    ...
    try:
        ...
        data = urllib.parse.urlencode(
            {"txtname": username, "txtpassword": password}
        ).encode()
        opener.open(
            urllib.request.Request(
                f"https://{host}:{WEBLCT_PORT}/weblct/TSLoginCheck", data=data
            )
        )
        time.sleep(2)
        req = urllib.request.Request(
            f"https://{host}:{WEBLCT_PORT}/weblct/neListServlet", data=b""
        )
        for attempt in range(3):
            ...
            if (
                error_elem is None
                or "busy" not in error_elem.get("errorinfo", "").lower()
            ):
                break
            time.sleep(5)  # Увеличили задержку
        else:
            raise HTTPException(
                status_code=500, detail=f"WebLCT error: {error_elem.get('errorinfo')}"
            )
        out = []
