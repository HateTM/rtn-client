...
def api_find_devices(
    ...
    try:
        ...
        # Пробуем GET, так как POST может требовать CSRF-токен
        resp = session.get(f"https://{host}:{WEBLCT_PORT}/weblct/neListServlet?sfid=280&flag=1", timeout=5, allow_redirects=True)
